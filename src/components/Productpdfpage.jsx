...
const drawPage = async (doc, p, idx, total, opts) => {
  ...
  // const stockLabel = stockQty > 10 ? "EN STOCK" : stockQty > 0 ? "STOCK LIMITÉ" : "RUPTURE";
  ...
  if (HAS_PRICE) {
    ...
    doc.text(unite || "unité(s)", 7 + doc.getTextWidth(String(stockQty)) * 1.18 + 1, BOT_Y + 23);

    /* Badge */
    // const bw = doc.getTextWidth(stockLabel) * 0.72 + 6;
    // sf(doc, stockColor); doc.roundedRect(HALF - bw - 5, BOT_Y + 4, bw, 6.5, 1.2, 1.2, "F");
    // doc.setFontSize(5.5); doc.setFont("helvetica", "bold"); st(doc, C.white);
    // doc.text(stockLabel, HALF - bw / 2 - 5 + bw / 2, BOT_Y + 8.7, { align: "center" });
    ...
    doc.text(unite || "unité(s)", 7 + doc.getTextWidth(String(stockQty)) * 1.18 + 2, BOT_Y + 24);

    // const bw = doc.getTextWidth(stockLabel) * 0.72 + 6;
    // sf(doc, stockColor); doc.roundedRect(PW - bw - 8, BOT_Y + 4, bw, 6.5, 1.2, 1.2, "F");
    // doc.setFontSize(5.5); doc.setFont("helvetica", "bold"); st(doc, C.white);
    // doc.text(stockLabel, PW - bw / 2 - 8 + bw / 2, BOT_Y + 8.7, { align: "center" });
  }
